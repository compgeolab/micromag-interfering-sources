...
\subsubsection{Simplex optimization} 
    ...
    %     {b_z}^{dip} = \frac{\mu_0}{4 \pi} \left [ 3 \frac{ (z-z_c) \left ( m_x (x-x_c) + m_y (y-y_c) + m_z (z-z_c) \right )}{((x-x_c)^2 + (y-y_c)^2 + (z-z_c)^2)^{5/2}} - \frac{m_z}{((x-x_c)^2 + (y-y_c)^2 + (z-z_c)^2)^{3/2}} \right ] .
    % \end{equation}
...
% \subsubsection{Calculation of theoretical magnetic signals} 
