...
The positions used in the inversion differ between the two methods. In the standard method, source locations are obtained directly from the homogeneous Euler deconvolution (Figure~\ref{inversion2}b), following the workflow defined by \citet{Souza-Junior2024}. In the iterative method, refined positions derived from the nonlinear inversion are used (Figure~\ref{inversion2}c). Figure~\ref{inversion2} presents a comparison between the estimated magnetic moment directions for the standard (b) and iterative (c) methods, relative to the true directions (a). The iterative method yields better overall agreement with the true directions, especially by recovering additional sources that the standard approach fails to detect, including those with weaker signals. This has the potential to improve the statistical robustness of the estimated directions, making the method particularly useful for applications in magnetic microscopy.
