...
%DIF DEL /tmp/WXTbx9jRR6/latexdiff-vc-submitted-jgr/manuscript.tex   Thu Apr  3 14:08:22 2025
%DIF ADD manuscript.tex                                              Fri Oct 24 16:37:21 2025
%
...
\DIFdelbegin \DIFdel{The first step in scientific data acquisition often involves analyzing entire samples , providing }\DIFdelend \DIFaddbegin \DIFadd{In paleomagnetism, bulk measurements of entire samples have traditionally been used to characterize remanent magnetization. While effective, this approach provides }\DIFaddend only a general \DIFdelbegin \DIFdel{characterization of the material. Enhancing data acquisition by improving spatial resolution and isolating the underlying phenomena contributing to the overall signal has become a central direction in various scientific fields. In paleomagnetism, this advancement is now possible with the advent of }\DIFdelend \DIFaddbegin \DIFadd{directional estimate and cannot resolve spatial variability or magnetization at the grain scale. Recent advances in }\DIFaddend magnetic microscopy (MM), \DIFdelbegin \DIFdel{whose }\DIFdelend \DIFaddbegin \DIFadd{due to its }\DIFaddend high spatial resolution and magnetic moment sensitivity\DIFdelbegin \DIFdel{allow for imaging samples at the mineral grain scale }\DIFdelend \DIFaddbegin \DIFadd{, now allow imaging at the scale of individual mineral grains}\DIFaddend . In this study, we aim to obtain reliable paleomagnetic directions using only MM data. To achieve this, we apply Euler deconvolution to solve the linear problem and mitigate the non-uniqueness associated with inversion. As an additional step, we refine the recovered parameters using a nonlinear inversion and remove interfering signals between sources to minimize noise. This algorithm was applied to both synthetic and real data and compared to its predecessor. The results from synthetic data demonstrate that this new approach is able to detect weaker sources and produce more accurate grain-level results, which in turn leads to larger datasets and improved statistical characterization of the sample. For real data, we observe that the iterative method was significantly more efficient than its predecessor, successfully retrieving the natural remanent magnetization direction of a basaltic sample with \ang{3} \DIFdelbegin \DIFdel{accuracy}\DIFdelend \DIFaddbegin \DIFadd{from the bulk measurement}\DIFaddend . This represents a significant step forward in applying MM data to paleomagnetic studies.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{Euler :}%DIFDELCMD < }
%DIFDELCMD <         %%%
\DIFdelend \DIFaddbegin \textbf{\DIFadd{Euler:}} \DIFadd{Estimate the source 3D position and base level by solving the Euler deconvolution equation \mbox{%DIFAUXCMD
\citep{Reid1990}}\hskip0pt%DIFAUXCMD
.
        }\DIFaddend \item \textbf{Linear magnetic moment inversion:} Perform a linear inversion to estimate the dipole moment of the source using the Euler deconvolution solution as prior information to fix the source position.
...
\subsection{Position estimation}
    ...
    Given $N$ observation of the magnetic field and its $x$-, $y$-, and $z$-derivatives, we can form a $N \times 4$ system of equations \DIFaddbegin \DIFadd{describing a single source dipole at location $(x_c,y_c,z_c)$ and constant offset $b$
    }\DIFaddend 
...
\subsection{Linear magnetic moment inversion}
    ...
    \noindent
    The background field $b$ estimated by Euler deconvolution is removed from the observed data to account for \DIFdelbegin \DIFdel{static }\DIFdelend \DIFaddbegin \DIFadd{constant }\DIFaddend shifts in the microscopy data which cannot be replicated by a dipolar field.
...
\subsection{Non-linear inversion} 
    ...
    To help mitigate this interference, \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{one can }\DIFaddend employ a non-linear inversion to estimate the dipole moment and source position simultaneously \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[\textit{e.g.}][]{Lima2016}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
    ...
    m_0 &= \sqrt{{m_x}_0^2+{m_y}_0^2+{m_z}_0^2}\ \DIFdelbegin \DIFdel{,
    }%DIFDELCMD < & {%%%
\DIFdel{m_x}%DIFDELCMD < }%%%
\DIFdel{^{j+1} }%DIFDELCMD < &%%%
\DIFdel{= \frac{{m_x}^{j}}{m_0},
    }%DIFDELCMD < & {%%%
\DIFdel{m_y}%DIFDELCMD < }%%%
\DIFdel{^{j+1} }%DIFDELCMD < &%%%
\DIFdel{= \frac{{m_y}^{j}}{m_0},
    }%DIFDELCMD < & %%%
\DIFdel{\text{and} }%DIFDELCMD < &
%DIFDELCMD <     & {%%%
\DIFdel{m_z}%DIFDELCMD < }%%%
\DIFdel{^{j+1} }%DIFDELCMD < &%%%
\DIFdel{= \frac{{m_z}^{j}}{m_0}
    }\DIFdelend %DIF > ,
    %DIF >  & {m_x}^{j+1} &= \frac{{m_x}^{j}}{m_0}, 
    %DIF >  & {m_y}^{j+1} &= \frac{{m_y}^{j}}{m_0}, 
    %DIF >  & \text{and} & 
    %DIF >  & {m_z}^{j+1} &= \frac{{m_z}^{j}}{m_0}
    \ .
    \end{align}

\noindent
Which was also applied for the position vector using the initial position \DIFdelbegin \DIFdel{estimates:
}\DIFdelend \DIFaddbegin \DIFadd{estimated using ED. %DIF > :
}\DIFaddend 

\DIFdelbegin \begin{align*}
\DIFdel{\label{normalizing_h_parameters}
}{\DIFdel{x_c}}\DIFdel{^{j+1} }&\DIFdel{= \frac{{x_c}^{j}}{{x_c}_0},
}& {\DIFdel{y_c}}\DIFdel{^{j+1} }&\DIFdel{= \frac{{y_c}^{j}}{{y_c}_0},
}& \DIFdel{\text{and} }&
&{\DIFdel{z_c}}\DIFdel{^{j+1} }&\DIFdel{= \frac{{z_c}^{j}}{{z_c}_0}
\ .
}\end{align*}%DIFAUXCMD
\DIFdelend %DIF >  \begin{align}
%DIF >  \label{normalizing_h_parameters}
%DIF >  {x_c}^{j+1} &= \frac{{x_c}^{j}}{{x_c}_0}, 
%DIF >  & {y_c}^{j+1} &= \frac{{y_c}^{j}}{{y_c}_0}, 
%DIF >  & \text{and} & 
%DIF >  &{z_c}^{j+1} &= \frac{{z_c}^{j}}{{z_c}_0}
%DIF >  \ .
%DIF >  \end{align}

\noindent
These normalization procedures ensure that each parameter falls within a unit range for a given number M of iterations of the simplex optimization, $j = 1,2,..., M$. \DIFaddbegin \DIFadd{Therefore, introducing a relative weight between position and moment in the optimization procedure — otherwise, the algorithm would prioritize changes in the position.
     }\DIFaddend 
...
\label{sec:signal_removal}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Background field correction:} In the proposed methodology, correction for the background field ($b$)\DIFdelbegin \DIFdel{eliminates the need for preprocessing the data for regional-residual separation to remove the effects of }\DIFdelend \DIFaddbegin \DIFadd{removes the need to account for baseline }\DIFaddend shifts in the magnetic data \DIFaddbegin \DIFadd{(}\textit{\DIFadd{e.g.,}} \DIFadd{long wavelength caused by deeper sources)}\DIFaddend . This application is feasible because $b$ is also provided by the ED solution when using windowed data. Simultaneous inversion of all sources would require the removal of the regional field prior to inversion, which is difficult to do without biasing the estimated directions.

        \item \textbf{No visible advantage over windowed data}: Tests with synthetic data \citep[see supplementary Jupyter notebooks in][]{figshare} showed no significant advantage over windowed data. The latter performed significantly better in the benchmark\DIFdelbegin \DIFdel{and had lower processing times}\DIFdelend .
...
\begin{enumerate}
    \item \textbf{Evaluating overlapping signal detection:} This test simulates a single map containing both strong and weak sources to evaluate the \DIFaddbegin \DIFadd{effectiveness of }\DIFaddend the new methodology \DIFdelbegin \DIFdel{'s effectiveness }\DIFdelend in discerning and accurately locating the weaker sources, despite \DIFdelbegin \DIFdel{the interferences causing }\DIFdelend \DIFaddbegin \DIFadd{interference caused }\DIFaddend by the stronger ones. \DIFaddbegin \DIFadd{The classification into "strong" and "weak" sources is based on the intensity assigned by the blob detection algorithm, which is approximately proportional to the amplitude of the magnetic signal.
}\DIFaddend 
...
The first model scenario consists of several dipole sources with varying moment magnitudes, inclinations, and declinations, organized into three distinct groups. The first group contains 150 dipoles with random orientations and moment magnitudes an order of magnitude larger than those in the second group. The second group comprises 50 dipoles with a stable orientation: an inclination of \(\ang{35}\) (\(\pm \ang{5}\)), a declination of \(\ang{340}\) (\(\pm \ang{5}\)), and dipole moment magnitudes centered at \(1.0 \times 10^{-16} Am^2\). Additionally, a third group includes 9 sporadic, deeper, and strongly magnetized sources with random orientations and significantly higher dipole moments of \(1.0 \times 10^{-11} Am^2\). \DIFdelbegin \DIFdel{Which }\DIFdelend \DIFaddbegin \DIFadd{This }\DIFaddend totals 209 magnetic particles.

To evaluate the methodologies, we simulate \DIFdelbegin \DIFdel{these }\DIFdelend magnetic sources randomly distributed \DIFaddbegin \DIFadd{(discrete uniform distribution) }\DIFaddend within a synthetic thin \DIFaddbegin \DIFadd{section }\DIFaddend with a field of view \DIFdelbegin \DIFdel{section measuring \(\qty{2000}{\micro\meter} \times \qty{2000}{\micro\meter}\)}\DIFdelend \DIFaddbegin \DIFadd{measuring \( \qty{2000}{\micro\meter} \times \qty{2000}{\micro\meter} \) and a thickness of \( \qty{20}{\micro\meter} \)}\DIFaddend . The synthetic vertical magnetic field data (\(b_z\)) are generated on a regular grid with \(\qty{2}{\micro\meter}\) spacing and a sensor sample distance of \(\qty{5}{\micro\meter}\), without any external applied field. Thus, the magnetic anomaly is solely caused by their dipole moment. \DIFdelbegin \DIFdel{High-frequency (\(\qty{50}{\nano\tesla}\) following \mbox{%DIFAUXCMD
\citet{Glenn2017}}\hskip0pt%DIFAUXCMD
)}\DIFdelend \DIFaddbegin \DIFadd{To simulate realistic measurement conditions}\DIFaddend , spatially correlated \DIFdelbegin \DIFdel{pseudo-random noise , modeled after the spectral characteristics of the blank }\DIFdelend \DIFaddbegin \DIFadd{high-frequency noise (\(\qty{50}{\micro\tesla}\), following \mbox{%DIFAUXCMD
\citet{Glenn2017}}\hskip0pt%DIFAUXCMD
) was added to the synthetic data. This noise was pseudo-random but spectrally shaped to match the characteristics of a blank QDM map (sample }\DIFaddend 2070615-NID08\DIFdelbegin \DIFdel{diamond map , was added to replicate typical experimental conditions of other diamonds, providing a realistic approximation of a QDM measurement}\DIFdelend \DIFaddbegin \DIFadd{), reproducing the typical background structure observed in real NV-diamond experiments}\DIFaddend . The modeled sources were positioned at depths ranging from 1 to \(\qty{20}{\micro\meter}\). To further emulate actual acquisition conditions, a positive baseline shift of \(\qty{400}{\nano\tesla}\) \citep[following][]{Hess2024} is applied to the magnetic field data. This setup tests the algorithm's robustness and accuracy when handling systematically shifted and noise-corrupted magnetic field measurements, as commonly observed in magnetic microscopy experiments.

The synthetic data inversion was \DIFdelbegin \DIFdel{resolved using both the standard method \mbox{%DIFAUXCMD
\citep{Souza-Junior2024} }\hskip0pt%DIFAUXCMD
and the newly proposed interfering sources methodology. A summary comparison of the classic Euler method and the iterative approach showed significant results in analyzing the }\DIFdelend \DIFaddbegin \DIFadd{performed using two approaches: the method previously proposed by \mbox{%DIFAUXCMD
\citet{Souza-Junior2024}}\hskip0pt%DIFAUXCMD
, which we will refer to as the standard method, and the new approach introduced in this study, hereafter referred to as the iterative method. A comparative summary between the standard and iterative approaches demonstrates the superior performance of the iterative method when applied to }\DIFaddend synthetic data. \DIFdelbegin \DIFdel{The iterative Euler deconvolution method notably enhanced source detectability, with newly identified windows highlighted (green squares , Figure\ref{euler2}a)}\DIFdelend \DIFaddbegin \DIFadd{By incorporating a second pass of source detection over the residual field, the iterative strategy significantly improved the recovery of weaker sources that were previously obscured. These newly identified sources are marked as green squares in Figure~\ref{euler2}a, highlighting the method's enhanced sensitivity and detection capability}\DIFaddend . Figure \ref{euler2}b displays the 112 sources \DIFdelbegin \DIFdel{initially detected}\DIFdelend \DIFaddbegin \DIFadd{detected, using the standard method, }\DIFaddend from the 209 modeled. Meanwhile, Figure \ref{euler2}c indicates that the iterative Euler method retrieved 166 sources. \DIFdelbegin \DIFdel{Initially, it may appear that the improvements of }\DIFdelend \DIFaddbegin \DIFadd{Although the improvements in source location accuracy provided by }\DIFaddend the iterative method over the standard \DIFdelbegin \DIFdel{one in terms of tridimensional source location precision were modest. Yet}\DIFdelend \DIFaddbegin \DIFadd{approach may appear modest}\DIFaddend , most previously identified sources \DIFdelbegin \DIFdel{demonstrate reduced position misfit (barring sporadic deeper ones)}\DIFdelend \DIFaddbegin \DIFadd{exhibit reduced position misfits}\DIFaddend . The larger \DIFdelbegin \DIFdel{misfits in Figure}\DIFdelend \DIFaddbegin \DIFadd{errors observed in Figure~}\DIFaddend \ref{euler2}c are \DIFdelbegin \DIFdel{linked to the }\DIFdelend \DIFaddbegin \DIFadd{primarily associated with }\DIFaddend newly identified grains (\DIFdelbegin \DIFdel{represented by the }\DIFdelend colored squares), \DIFdelbegin \DIFdel{as their smaller dipole moment results in greater error in the Euler estimation. This increase }\DIFdelend \DIFaddbegin \DIFadd{whose lower dipole moments lead to increased uncertainty in the Euler-based estimated position. This gain }\DIFaddend in accuracy is \DIFdelbegin \DIFdel{crucial, especially in scenarios where stronger magnetic sources can }\DIFdelend \DIFaddbegin \DIFadd{critical, especially when strong sources }\DIFaddend distort the magnetic field \DIFdelbegin \DIFdel{anomaly of weaker sources. Although this new methodology can markedly increase the accuracy in the estimated position for virtually all sources, the biggest errors are still related to clustered sources . Also, in }\DIFdelend \DIFaddbegin \DIFadd{anomalies of nearby weaker ones. The iterative method enhances location estimates for most sources, although closely spaced sources still pose a challenge. In }\DIFaddend both cases, \DIFdelbegin \DIFdel{the Euler deconvolution seems unaffected by the presence of a shift in the magnetic field}\DIFdelend \DIFaddbegin \DIFadd{Euler deconvolution remains robust to background field shifts. This improvement in accuracy is evaluated by comparing the estimated positions with the true synthetic source locations, as shown in figures~\ref{euler2}b and \ref{euler2}c, where the sources identified in the first round of blob detection (represented by circles) display colors closer to white tones, indicating smaller position misfits}\DIFaddend .
...
The \DIFdelbegin \DIFdel{iterative Euler estimated positions were }\DIFdelend \DIFaddbegin \DIFadd{positions }\DIFaddend used in the \DIFdelbegin \DIFdel{iterative magnetic inversion due to their superior accuracy. In contrast, }\DIFdelend \DIFaddbegin \DIFadd{inversion differ between the two methods. In }\DIFaddend the standard method\DIFdelbegin \DIFdel{relied on positions obtained from the original, unmodified algorithm. }\DIFdelend \DIFaddbegin \DIFadd{, source locations are obtained directly from the homogeneous Euler deconvolution (Figure~\ref{inversion2}b), following the workflow defined by \mbox{%DIFAUXCMD
\citet{Souza-Junior2024}}\hskip0pt%DIFAUXCMD
. In the iterative method, refined positions derived from the nonlinear inversion are used (}\DIFaddend Figure~\ref{inversion2}\DIFaddbegin \DIFadd{c). Figure~\ref{inversion2} }\DIFaddend presents a comparison \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{between }\DIFaddend the estimated magnetic moment directions for the standard (\DIFdelbegin \DIFdel{Figure~\ref{inversion2}}\DIFdelend b) and iterative (\DIFdelbegin \DIFdel{Figure~\ref{inversion2}}\DIFdelend c) methods, \DIFdelbegin \DIFdel{in relation }\DIFdelend \DIFaddbegin \DIFadd{relative }\DIFaddend to the true directions (\DIFdelbegin \DIFdel{Figure~\ref{inversion2}}\DIFdelend a). \DIFdelbegin \DIFdel{As expected, the iterative method provides a }\DIFdelend \DIFaddbegin \DIFadd{The iterative method yields }\DIFaddend better overall agreement with the true directions, \DIFdelbegin \DIFdel{recovering a larger number of sources with reliable estimates. The improvement is particularly noticeable as the iterative approachis capable of identifying signals from weaker particles, increasing the number of recovered sources }\DIFdelend \DIFaddbegin \DIFadd{showing an improvement of approximately 48\% relative to the standard approach. This enhancement is mainly due to its ability to recover additional sources that the standard method fails to detect, including those characterized by weaker magnetic signals}\DIFaddend . This has the potential to improve the statistical robustness of the estimated directions, making the method particularly useful for applications in magnetic microscopy.
...
\begin{figure}[tb!]
  ...
  \caption{
    Comparison of true and estimated dipole magnetic moments and directions for the overlapping signals in the synthetic sample. Panel (a) shows the true modeled direction, while panels (b) and (c) display the estimated vector directions obtained using the standard ($M = 112$) and iterative ($M = 166$) methods, respectively. Results for both methods were filtered based on the coefficient of determination ($R^2 \geq 0.9$)\DIFaddbeginFL \DIFaddFL{, filtering sources with at least 90\% of their anomaly explained by a dipole \mbox{%DIFAUXCMD
\citep[\textit{e.g.,}][]{Souza-Junior2024, Fu2020}}\hskip0pt%DIFAUXCMD
}\DIFaddendFL . Directions are hued by \DIFaddbeginFL \DIFaddFL{the }\DIFaddendFL magnitude of their magnetic moments. Open/closed circles indicate positive/negative inclinations.
  }
  \label{inversion2}
\end{figure}
...
The directional parameters and magnetic moment intensities distributions were kept constant across all simulations, as the goal was to isolate the effects of increasing particle density. For each simulation, the total \(N\) magnetic dipole sources were created and randomly distributed across a thin section of \qty{2000}{\um} \(\times\) \qty{1400}{\um} \DIFaddbegin \DIFadd{\(\times\) }\qty{20}{\um}\DIFaddend . The dipole moments were sampled from a lognormal distribution centered at a mean amplitude of \(1 \times 10^{-14}\,\mathrm{A \cdot m^2}\), with a standard deviation spanning two orders of magnitude. This configuration ensures that most of the distribution is concentrated around the mean, while also allowing for the presence of particles with magnetic moments as strong as \(10^{-11}\,\mathrm{A \cdot m^2}\), simulating larger particles with a greater influence on the magnetic field compared to weaker particles. The modeled sources were also positioned at pseudo-random depths ranging from 1 to \(\qty{20}{\micro\meter}\).

A variety of geological processes can lead to the acquisition of natural remanent magnetization (NRM). If the NRM is primary—acquired at the same time as the rock \DIFaddbegin \DIFadd{formation}\DIFaddend —it will generally align with the ambient magnetic field, such as a planetary field. However, the alignment of individual grains is influenced by multiple factors, including particle properties \citep[shape, size, domain state,][]{Bellon2025} and the conditions under which NRM is acquired, such as the cooling rate of lavas. As a result, \DIFdelbegin \DIFdel{while individual grain signals may show a general tendency toward alignment, the magnetic moments of many particles will still exhibit some degree of random orientation}\DIFdelend \DIFaddbegin \DIFadd{although each grain tends to align its remanent magnetization along the easy axis nearest to the external field, the distribution of easy-axis orientations leads to a degree of apparent randomness in the assemblage of directions}\DIFaddend . However, modeling TRM directions is not a straight-forward task, instead we can mirror an isothermal remanent magnetization (IRM) behavior in our synthetic data. Thus by setting the inclination and declination of directional parameters primarily at 30° and \DIFdelbegin \DIFdel{330}\DIFdelend \DIFaddbegin \DIFadd{30}\DIFaddend °, respectively, and 50° dispersion angle introduced variability within spherical statistics, where directional data extends over a unit sphere rather than Cartesian coordinates. This angle indicates a broad spread of directions around the mean, accounting for significant variability in dipole directions, which is crucial for accurately representing the natural diversity of magnetic sources.

The synthetic magnetic field \(b_z\) was computed over a grid with a \qty{2}{\um} spacing, and the measurement plane was set at a constant height of \qty{5}{\um} above the thin section. The same high-frequency QDM spatially correlated pseudo-random noise was added to the computed field to mimic realistic measurement conditions. Additionally, a positive baseline shift of \(\qty{400}{\nano\tesla}\) \citep[following][]{Hess2024} was introduced to replicate the acquisition process in magnetic microscopy \DIFaddbegin \DIFadd{using QDM}\DIFaddend . The simulations were conducted in a zero external field environment, where all signals generated in the maps were solely and exclusively caused by the magnetization of the simulated particles, simulating real acquisitions typically performed in magnetically shielded rooms to eliminate the influence of the Earth's geomagnetic field. A total of 10 simulations were carried out for each different particle density level.

The results reveal a clear difference in the performance of the standard (red curves and left stereoplots) and iterative (blue curves and right stereoplots) methods in reconstructing the synthetic IRM direction imparted in the simulations. As shown in Figure~\ref{synthetic-data-stereograms}a, for \DIFdelbegin \DIFdel{lower densities }\DIFdelend \DIFaddbegin \DIFadd{the lower density }\DIFaddend of particles, both methods perform almost the same, given that in this case, the amount of interference between the sources is small. \DIFdelbegin \DIFdel{This is also observed for the mid-term density (Figure~\ref{synthetic-data-stereograms}b). }\DIFdelend However, for the higher density of magnetic particles (Figure~\ref{synthetic-data-stereograms}\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{b}\DIFaddend ), the iterative method starts to outperform the standard method. \DIFdelbegin \DIFdel{This shows the superiority of the iterative technique, as there are no randomly stronger particles simulating different viscous directions. Even though there are }\DIFdelend \DIFaddbegin \DIFadd{Overall, the iterative method recovered nearly twice as many anomalies as the standard approach, demonstrating its clear advantage in resolving both strong and weak sources. Despite the presence of }\DIFaddend many strong sources, they all follow \DIFdelbegin \DIFdel{the same }\DIFdelend \DIFaddbegin \DIFadd{a consistent }\DIFaddend distribution around the synthetic direction, \DIFdelbegin \DIFdel{similar to what happens in an IRM induced sample}\DIFdelend \DIFaddbegin \DIFadd{as seen in IRM-induced samples}\DIFaddend . This could explain, alongside the low density of particles, the good results presented by the \DIFdelbegin \DIFdel{real carbonatic data presented by \mbox{%DIFAUXCMD
\citet{Souza-Junior2024}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{carbonatic sample \mbox{%DIFAUXCMD
\citep[presented by][]{Souza-Junior2024}}\hskip0pt%DIFAUXCMD
}\DIFaddend , which was subjected to strong IRM\DIFdelbegin \DIFdel{, though it remains ambiguous }\DIFdelend \DIFaddbegin \DIFadd{. However, }\DIFaddend whether the results are due to the low particle density or the IRM \DIFaddbegin \DIFadd{remains ambiguous}\DIFaddend . For this reason, the next tests involved samples \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{carrying a }\DIFaddend natural remanent magnetization.

\begin{figure}[tb!]
  \centering
  \includegraphics[width=1.0\linewidth]{figures/synthetic-different-densities-stereoplot.png}
  \caption{
Comparison of the magnetic directions obtained using the original (red line and left stereogram) and the iterative (blue line and right stereogram) methods on each synthetic grain \DIFdelbeginFL \DIFdelFL{distributions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{distribution }\DIFaddendFL simulation. Panel (a) shows the angular misfit between the cumulative vector and the measured magnetic direction for the low-density distribution. Panel (b) illustrates the angular misfit for the \DIFdelbeginFL \DIFdelFL{moderate-density distribution. Finally, panel (c) presents the results for }\DIFdelendFL high-density distribution. The stereographic projections of the filtered magnetic vectors ($R^2 \geq 0.9$) for each density are also shown, with modeled bias direction (star) and the cumulative direction (square) indicated. The color gradient represents the logarithmic scale of particle counts, with warmer colors indicating higher particle concentrations.
  }
...
\DIFaddbegin \DIFadd{For both samples, all magnetic vectors associated with the identified anomalies were compiled into a database. This dataset was filtered based on the model’s coefficient of determination ($R^2$), retaining only vectors with $R^2 \geq 0.9$. This filtering step is important to remove inversion results that do not comply with the dipolar assumption \mbox{%DIFAUXCMD
\citep[\textit{e.g.,}][]{Souza-Junior2024, Fu2020}}\hskip0pt%DIFAUXCMD
. An $R^2 \geq 0.9$ ensures that only results closely fitting the data are kept (the maximum value of $R^2$ is 1). Outliers were further removed by excluding values exceeding 1.5 times the third quartile of the dipole moment intensities. The selected vectors were then ordered by magnetic moment intensity and progressively summed. At each step, the cumulative vector was compared with the NRM measured for the whole sample.
}

\DIFaddend \subsection{Evaluation with a stable, dispersed assembly}

To evaluate the feasibility of magnetic microscopy techniques in detecting and characterizing TRM, we selected a well-preserved fragment of baked clay pavement tile (sample RSLG1) from the archaeological site São Luiz Gonzaga reduction (1657-1687 AD). This fragment was studied by \citet{Poletti2016}. According to these authors, the magnetic properties of the sample indicate that magnetization is carried by a low-coercivity phase, most likely PSD Ti-poor titanomagnetite. Their interpretation is supported by the IRM curves saturated at fields up to approximately 0.3 T, narrow hysteresis loops, and maximum NRM demagnetization temperatures around 550°C. \DIFdelbegin \DIFdel{First order }\DIFdelend \DIFaddbegin \DIFadd{First-order }\DIFaddend reversal curves (FORC) also confirm a predominance of non-interacting SD/PSD grains. 
\DIFdelbegin \DIFdel{These characteristics make the sample an ideal candidate for magnetic microscopy studies aimed at investigating its natural remanent magnetization.
}\DIFdelend 

%DIF >  These characteristics make the sample an ideal candidate for magnetic microscopy studies aimed at investigating its natural remanent magnetization.
\DIFaddbegin 

\DIFaddend We mapped the NRM of the thin-section sample using the QDM at the Harvard Paleomagnetics Lab, Department of Earth and Planetary Sciences, Harvard University. All QDM data were taken in projected magnetic microscopy (PMM) mode and converted to the vertical component of the magnetic field ($b_z$) using a spectral approach \citep{Fu2020, Glenn2017, Lima2009}. We applied a 0.9 mT bias magnetic field during measurement, which was reversed periodically to result in a net bias field of ~400 nT\DIFaddbegin \DIFadd{, as shown by \mbox{%DIFAUXCMD
\citet{Hess2024}}\hskip0pt%DIFAUXCMD
}\DIFaddend . A total of 20 randomly selected regions were subjected to this procedure to ensure representative coverage of the sample's magnetic properties, in which Figure~\ref{real-data-maps}a showcases one of the maps obtained. The \DIFdelbegin \DIFdel{scanned area measured $\qty{1410}{\um} \times \qty{2256}{\um}$ }\DIFdelend \DIFaddbegin \DIFadd{imaged area covered \(\qty{1410}{\um} \times \qty{2256}{\um}\) }\DIFaddend with a grid spacing of \DIFdelbegin %DIFDELCMD < \qty{2.35}{\um} %%%
\DIFdel{($N = 576 \times 10^{3}$)}\DIFdelend \DIFaddbegin \DIFadd{\(\qty{2.35}{\um}\), corresponding to approximately \(5.76\times10^{5}\) measurement points}\DIFaddend . Data acquisition was conducted at a constant sensor-sample distance of \DIFdelbegin %DIFDELCMD < \qty{1} %%%
\DIFdel{- }\DIFdelend \qty{5}{\um}. This allowed for high-resolution mapping of the magnetic field distribution.
...
%DIFDELCMD < %%%
\DIFdelend The comparison between the standard and iterative algorithms \DIFdelbegin \DIFdel{highlights significant }\DIFdelend \DIFaddbegin \DIFadd{reveals substantial }\DIFaddend differences in their ability to reconstruct the NRM of the sample. As shown in Figure~\ref{ceramic-data-stereograms}, the angular misfit between the cumulative magnetic vector and the measured NRM decreases more effectively with the iterative approach\DIFdelbegin \DIFdel{. We further remove outliers in the dataset by excluding values that are 1.5 times larger than the third-quartile of the dipole moment intensities. While the non-filtered standard method results in a consistently higher misfit , stabilizing around \mbox{%DIFAUXCMD
\ang{55}}\hskip0pt%DIFAUXCMD
, the iterative method quickly refines the alignment, reducing the misfit to approximately \mbox{%DIFAUXCMD
\ang{10} }\hskip0pt%DIFAUXCMD
within a few hundred particles. While for filtered curves this misfit difference reduces, there are virtually no changes in the iterative method result. This indicates that the iterative approach is more efficient in capturing the true remanent magnetization with fewer contributing grains. The stereographic projections further support this observation by illustrating the directional distribution of the filtered vectors for each method, where the iterative approach yields a distribution more closely aligned with the measured NRM (star symbol) and the cumulative direction (square symbol). These results demonstrate that the iterative method significantly enhances the accuracy of NRM reconstruction by effectively mitigating }\DIFdelend \DIFaddbegin \DIFadd{, which stabilizes around \mbox{%DIFAUXCMD
\ang{10}}\hskip0pt%DIFAUXCMD
. In contrast, the standard method converges more slowly and remains at a higher misfit of approximately \mbox{%DIFAUXCMD
\ang{16}}\hskip0pt%DIFAUXCMD
. In addition to achieving better directional agreement, the iterative method recovered nearly twice as many magnetic anomalies (\(N=4655\)) compared to the standard approach (\(N=2130\)). Overall, these results emphasize the superiority of the iterative inversion in resolving both strong and weak sources while reducing }\DIFaddend directional dispersion.

\DIFdelbegin \DIFdel{The limited improvement threshold in directional accuracy, even as more particles are added, is a noteworthy observation. We propose two possible contributing factors. First, as previously discussed, the bias of individual particles towards the magnetic field is only at the level of $\sim1\%$. When measuring the total NRM at the macroscopic scale, the contributions of all grains are summed, allowing randomly oriented signals to cancel out while the more aligned grains }\DIFdelend %DIF >  The limited improvement threshold in directional accuracy, even as more particles are added, is a noteworthy observation. We propose two possible contributing factors. First, as previously discussed, the bias of individual particles towards the magnetic field is only at the level of $\sim1\%$. When measuring the total NRM at the macroscopic scale, the contributions of all grains are summed, allowing randomly oriented signals to cancel out while the more aligned grains enhance the overall alignment with the field direction. Classical rock thin sections, like the one used in this study, are typically prepared at a thickness of around \qty{30}{\um} to facilitate the identification of mineral features under a petrographic microscope. Within such a thin section, millions of magnetic minerals are likely present. However, in MM data acquisition, we only sample a small portion of the entire thin section. Even within this measured area, not all magnetic signals are fully detected, as the measured magnetic moment depends on both the depth of the particles and their magnetic strength. As a result, when more particle directions are incorporated into the combined magnetic signal, a saturation effect may occur. Beyond a certain threshold, the inclusion of additional, randomly oriented signals could outweigh the contribution of well-aligned grains, limiting further improvement in directional accuracy. This would explain why, despite increasing the number of particles, the expected refinement in the reconstructed direction is not observed.
\DIFaddbegin 

\DIFadd{First, although individual particles exhibit a weak statistical bias toward the external magnetic field during remanence acquisition, this bias only contributes meaningfully to the net signal when a representative population of grains is sampled. Ideally, in an isotropic and randomly oriented assemblage, increasing the number of particles should reduce noise through cancellation of random directions and }\DIFaddend enhance the overall alignment \DIFdelbegin \DIFdel{with the field direction. Classical rock thin sections, like the one used in this study, are typically prepared at a thickness of around }%DIFDELCMD < \qty{30}{\um} %%%
\DIFdel{to facilitate the identification of mineral features under a petrographic microscope. Within such a thin section, millions of magnetic minerals are likely present. }\DIFdelend \DIFaddbegin \DIFadd{by reinforcing the contributions from well-aligned grains. }\DIFaddend However, \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{not all natural samples might meet these ideal conditions. Anisotropies in the rock fabric can lead to preferred grain alignments, introducing a systematic directional bias. Moreover, mixtures with certain magnetic minerals, such as hematite, possess strong magnetocrystalline anisotropy (e.g., a basal preference), which can further constrain the directions along which remanence is acquired. These effects can cause specific orientations to be overrepresented in the magnetic signal, meaning that the addition of more particles may amplify these inherent biases rather than average them out. Besides, }\DIFaddend MM data acquisition \DIFdelbegin \DIFdel{, we only sample a small }\DIFdelend \DIFaddbegin \DIFadd{samples only a limited }\DIFaddend portion of the \DIFdelbegin \DIFdel{entire }\DIFdelend thin section. \DIFdelbegin \DIFdel{Even within this measured area }\DIFdelend \DIFaddbegin \DIFadd{Although a standard \(\approx 30 \mu m\) thin section may contain millions of grains, the imaged area captures only a small subset. Furthermore}\DIFaddend , not all \DIFdelbegin \DIFdel{magnetic signals are fully detected, as }\DIFdelend \DIFaddbegin \DIFadd{grains contribute equally: }\DIFaddend the measured magnetic moment depends on both the \DIFdelbegin \DIFdel{depth of the particles and their magnetic strength. As a result, when more particledirections are incorporated into the combined magnetic signal, a saturation effect may occur. Beyond a certain threshold, the inclusion of additional, randomly oriented signals could outweigh the contribution of well-aligned grains, limiting further }\DIFdelend \DIFaddbegin \DIFadd{strength of the particle’s remanence and its proximity to the sensor, meaning deeper or weaker grains may be underrepresented. As a result, the summation of signals may be incomplete and structurally biased, ultimately limiting the }\DIFaddend improvement in directional accuracy \DIFdelbegin \DIFdel{. This would explain why, despite increasing the number of particles , the expected refinement in the reconstructed direction is not observed}\DIFdelend \DIFaddbegin \DIFadd{even as more particles are added}\DIFaddend .

\DIFdelbegin \DIFdel{The second hypothesis involves }\DIFdelend \DIFaddbegin \DIFadd{Another contributing factor involves potential }\DIFaddend human-induced errors during the measurement process. The sample had to be manually positioned and oriented in both the QDM and the 2G magnetometer, introducing potential sources of misalignment. In the 2G magnetometer, the sample was measured in a vertical orientation and later rotated to match the reference frame of the thin section used in the QDM analysis. This manual handling and reorientation may have introduced small but cumulative misalignments, contributing to the observed directional discrepancies. Another potential source of bias in the 2G magnetometer measurements could be contamination, either from the glass slide or another external source near the sample.

\begin{figure}[tb!]
  \centering
  \includegraphics[width=1\linewidth]{figures/ceramic-data-stereoplot.png}
  \caption{
  Comparison of the standard (red) and iterative (blue) algorithms in reconstructing the NRM direction from the ceramic sample. (Left) Angular misfit between the cumulative vector and the measured NRM as a function of the number of particles ($N$). (Right) Stereographic projections of the filtered magnetic vectors ($R^2 \geq 0.9$) for both methods, with the measured NRM (star) \DIFdelbeginFL \DIFdelFL{, non-filtered cumulative direction (triangle), }\DIFdelendFL and the \DIFdelbeginFL \DIFdelFL{filtered }\DIFdelendFL cumulative direction (square) indicated. The color gradient denotes the logarithmic scale of particle counts, with warmer colors representing higher concentrations.
  \DIFdelbeginFL \DIFdelFL{The solid-lined and dashed-lined curves represent the non-filtered and outliers filtered data, respectively.
  }\DIFdelendFL }
...
The acquisition of QDM data was essentially the same as the aforementioned experiment with the ceramic tile. The QDM \DIFdelbegin \DIFdel{scans were }\DIFdelend \DIFaddbegin \DIFadd{imaging was }\DIFaddend performed to capture the magnetic field distribution at high spatial resolution. To better compare the results with the \DIFdelbegin \DIFdel{tile }\DIFdelend \DIFaddbegin \DIFadd{ceramic }\DIFaddend sample, the same number of randomized mapping spots was performed. Figure~\ref{real-data-maps}b shows one example of the maps obtained. The resulting data enabled the calculation of angular misfit as a function of the number of particles, providing insight into the reliability of different analytical methods.

The results reveal a \DIFdelbegin \DIFdel{clear difference in the performance of the }\DIFdelend \DIFaddbegin \DIFadd{consistent difference between the }\DIFaddend standard and iterative methods in reconstructing the NRM direction. As shown in Figure~\ref{basalt-data-stereograms}, \DIFdelbegin \DIFdel{the iterative method consistently achieves }\DIFdelend \DIFaddbegin \DIFadd{both methods eventually stabilize, but the iterative approach reaches }\DIFaddend lower angular misfit values \DIFdelbegin \DIFdel{, before and after the intensity filtering, as more particles are incorporated , demonstrating improved accuracy and stability. In contrast, while the standard methodalso reduces the misfit over time, especially after the filtering, it stabilizes around 5 degrees only after approximately a thousand grains. The stereographic projections further illustrate these differences. The standard method exhibits a wider spread of vectors, suggesting that many individual particle contributions remain misaligned. Meanwhile, the iterative method results in a more tightly clustered distribution, with the misfit tending toward zero as the number of particles increases (although there is also, seemingly, a stagnation threshold) . This suggests that the iterative approach is more effective in refining the directional signal, leading to greater precisionin the reconstructed magnetization}\DIFdelend \DIFaddbegin \DIFadd{and maintains them more steadily across the entire range of incorporated particles. The standard method, in contrast, stabilizes at higher misfit levels, indicating a less accurate directional reconstruction. In addition, the iterative approach recovers nearly three times as many magnetic sources (\(N=7265\)) as the standard method (\(N=2625\)), demonstrating its enhanced ability to resolve weaker or previously undetected anomalies while maintaining higher directional precision}\DIFaddend .
...
Inverting \DIFdelbegin \DIFdel{unique magnetic moment }\DIFdelend \DIFaddbegin \DIFadd{magnetic moments }\DIFaddend from MM data \DIFdelbegin \DIFdel{, regardless of the technique, demands knowing the position of sources. The }\DIFdelend \DIFaddbegin \DIFadd{often requires some information about the position or characteristics of the sources, which can be obtained through the }\DIFaddend application of the Euler deconvolution technique\DIFdelbegin \DIFdel{emerges to avoid additional measurements, such as nano-tomography, to uncover that information}\DIFdelend . The isolated windows approach, as \DIFdelbegin \DIFdel{justified }\DIFdelend \DIFaddbegin \DIFadd{introduced }\DIFaddend by \cite{Souza-Junior2024}, enables the separation of the primary signal from the surrounding area using the total gradient anomaly. This method effectively isolates the desired signal while excluding regions outside the window boundaries, which are less sensitive to variations in the magnetic parameters of the specific source. Using this \DIFdelbegin \DIFdel{windows }\DIFdelend \DIFaddbegin \DIFadd{window }\DIFaddend approach enables a rapid solution to the inversion problem while keeping it \DIFdelbegin \DIFdel{largely }\DIFdelend overdetermined, since only three parameters are solved per sliced data. However, this thresholding exclusion of the area from the inversion domain violates the basic theory of inversion, which demands that the entire system be considered, ensuring that the \DIFdelbegin \DIFdel{interactions }\DIFdelend \DIFaddbegin \DIFadd{superposition of the signals }\DIFaddend between all sources \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend properly accounted for to obtain a unique and precise solution \citep{Baratchart2013, Lima2013}. Thus, the previous approach of \cite{Souza-Junior2024} will not yield reliable results in all cases. Specifically, in cases influenced by distant sources, even those that are weak, either within the window or close by, which compromises its effectiveness. The latter reflects \DIFdelbegin \DIFdel{on a higher amount }\DIFdelend \DIFaddbegin \DIFadd{a higher number }\DIFaddend of results discarded by the filtering criterion. 

To tackle these challenges, the iterative method was introduced, providing a more reliable solution for mitigating interference between sources while still preserving the isolated windows approach. Naturally, this comes at the cost of increased inversion time, but the method remains efficient enough to run on a personal computer without demanding massive computational power. Despite the improvements, not all problems have been solved. The \DIFdelbegin \DIFdel{new method }\DIFdelend \DIFaddbegin \DIFadd{iterative approach }\DIFaddend is more effective at detecting \DIFdelbegin \DIFdel{particles }\DIFdelend \DIFaddbegin \DIFadd{anomalies }\DIFaddend and results in more \DIFdelbegin \DIFdel{particles }\DIFdelend \DIFaddbegin \DIFadd{of them }\DIFaddend passing through the filtering criteria. However, it still faces challenges with inherent limitations of the Euler deconvolution, such as clustered signals. This introduces position estimation errors that are directly influenced by the particle's magnetic signal strength. In particular, errors in vertical position estimation lead to a compensatory trade-off between this positional uncertainty and the recovered dipole intensity. \DIFdelbegin \DIFdel{For this reason, our }\DIFdelend \DIFaddbegin \DIFadd{The latter could benefit from the refined implementation of an approach to solving Euler's homogeneity equation, as proposed by \mbox{%DIFAUXCMD
\citet{Uieda2025}}\hskip0pt%DIFAUXCMD
.
}

\subsection{\DIFadd{Directional recovery assessment}}

\DIFadd{Our }\DIFaddend focus here is on the directional aspects of the dataset, \DIFdelbegin \DIFdel{while the intensity aspect will require more complex future experiments , along with the implementation of a more refined approach to solving Euler's homogeneity equation, as proposed by \mbox{%DIFAUXCMD
\citet{Uieda2025}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{as the recovered directions from QDM data represent a weighted sum of all contributing vectors within each window and can be meaningfully compared to bulk magnetization measurements. In contrast, intensity values are not directly comparable, since the total moment in bulk samples arises from a vastly larger number of sources—many of which may not be detected by magnetic microscopy due to the limited imaging area and the fact that the detected signal predominantly originates from near-surface grains. Addressing this discrepancy will require future experiments focused on calibration or normalization strategies to bridge the gap between QDM-derived intensities and bulk measurements}\DIFaddend .

\DIFdelbegin \subsection{\DIFdel{Directional recovery assessment}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend As noted by \citet{Oliveira2015Estimation}, the least-squares estimator is less sensitive to small errors in particle position and violation of the dipolar assumption when recovering directional information (declination and inclination). This makes the new method particularly effective in refining estimates, increasing the number of results that meet the filtering criteria. As a result, the approach becomes more statistically robust, ultimately improving the overall data quality. The fitting performance was assessed using both synthetic and real datasets, requiring a reliable reference for comparison. In the case of synthetic data, a directional bias (synthetic \DIFdelbegin \DIFdel{NRM}\DIFdelend \DIFaddbegin \DIFadd{IRM}\DIFaddend ) was introduced to the dipole magnetic moments based on a spherical distribution. For real samples, the reference direction was obtained from NRM measurements of the whole thin section using the 2G magnetometer.

From the synthetic tests (Figure~\ref{synthetic-data-stereograms}\DIFdelbegin \DIFdel{a-c}\DIFdelend ), both the original \citep{Souza-Junior2024} and iterative methods performed well in recovering the directional bias mainly for two reasons. First, dipolar models are generally simpler and tend to produce more results that pass the filtering criteria. Even though the standard method is less efficient in estimating parameters, this deficiency is compensated by the overall behavior of the synthetic sample, and any poor-fitting is treated as random inputs that are effectively removed in the vector sum. The second reason is that, despite a wide distribution of particle moments (from \(10^{-15}\) to \(10^{-11}\)), the stronger particles, which could have further complicated the signal, also followed the bias direction. This contrasts with the overlapping signals synthetic case (Section~\ref{sec:synthetic-overlapping}), where the strong particles were randomly oriented. Nonetheless, it serves as a proxy to isothermal remanent magnetization study cases.
...
Our improved algorithm demonstrates substantial advancements over its predecessor, particularly in the context of magnetic microscopy mapping for paleomagnetic studies. By refining the isolation of the primary signal and incorporating an \DIFdelbegin \DIFdel{interfering source }\DIFdelend \DIFaddbegin \DIFadd{interfering-source }\DIFaddend algorithm, we have enhanced the accuracy of 3D positioning and dipole moment estimations, addressing key limitations of the previous version. These improvements have led to better particle distribution and an increased number of particles meeting the filtering criteria, while also identifying more grains through re-detection. \DIFaddbegin \DIFadd{Based on both synthetic and real samples, the iterative method detects approximately two to three times more magnetic anomalies compared to the standard approach, when applying an \(R^{2} \geq 0.9\) threshold. This demonstrates that the iterative workflow not only increases the number of detected sources but also yields better-resolved and more reliable estimates. }\DIFaddend Consequently, the cumulative vector analysis now includes more grains, ensuring statistically reliable direction estimates. This can be achieved with particle counts ranging from several hundred to a few thousand, as demonstrated by both numerical simulations and real data. 
\DIFdelbegin \DIFdel{However, similar conclusions cannot be drawn for intensity studies, as limitations persist in the recovered moment for grains, particularly in clustered particles. Therefore, further research is needed to assess whether this enhanced technique can be applied to paleointensity studies. Despite these challenges, the present work represents a significant advancement in paleomagnetic research at the microscale, owing to the algorithm's ability to recover bulk directional information.
}\DIFdelend 

%DIF >  \textcolor{red}{However, similar conclusions cannot be drawn for intensity studies, as limitations persist in the recovered moment for grains, particularly in clustered particles. Therefore, further research is needed to assess whether this enhanced technique can be applied to paleointensity studies. Despite these challenges, the present work represents a significant advancement in paleomagnetic research at the microscale, owing to the algorithm's ability to recover bulk directional information.}
\DIFaddbegin 

\DIFaddend This provides an important step towards paleomagnetism using magnetic microscopy. The possibility of retrieving reliable magnetization directions from thin sections broadens the horizons of previous bulk-derived measurements, such as paleomagnetic tests (e.g., ``conglomerate'' and microscale fold tests). Not only this, we have more capability to spatially correlate the directions estimated (e.g. single crystal inclusion) by obtaining the magnetic information associated with optical images.


\section{Open research}

The Python source code used to produce all results and figures presented here, as well as supplementary figures and Jupyter notebooks, and the QDM magnetic microscopy data used in this study are available from \citet{figshare}, which can also be found on \url{https://github.com/\GitHubRepository} under the MIT (source code)\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend CC-BY (\DIFdelbegin \DIFdel{data, text , and figures}\DIFdelend \DIFaddbegin \DIFadd{text and figures), and CC-0 (data}\DIFaddend ) licenses.
